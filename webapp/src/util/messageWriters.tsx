...
const getDiscordSlotText = (slot: Slot, pingDj: boolean = false): string => {
    ...
    if (pingDj) {
        return `${slot.start_time ? dateToDiscordTime(slot.start_time) : ""} : <@${slot.discord_id}> [${slot.name}]${debuttText}`;
    } else {
        return `${slot.start_time ? dateToDiscordTime(slot.start_time) : ""} : ${slot.name}${debuttText}`;
    }
}
